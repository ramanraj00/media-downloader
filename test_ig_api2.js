...
const { getPostgresClient } = require('./packages/db/dist/client.js');
const Redis = require('ioredis');
const fetch = require('node-fetch');

async function main() {
  const dbClient = getPostgresClient(process.env.DATABASE_URL || 'postgresql://media_dl:dev_password@localhost:5432/media_dl');
  const redis = new Redis(process.env.REDIS_URL || 'redis://localhost:6379');
  
  const pool = new CredentialPool(dbClient, redis);
