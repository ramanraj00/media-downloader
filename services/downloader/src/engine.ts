...
async function executeDirect(
  ...
  try {
    ...
    const extResult = await breaker.execute(
      () => adapter.extract(job.url, outputDir, encryptedData),
      (err) => err instanceof DatacenterBlockedError || err instanceof TransientError
    );
...
async function executeEgress(
  ...
  for (let attempt = 0; attempt < TIER3_MAX_EGRESS_ATTEMPTS; attempt++) {
    ...
    if (!egressAcq) {
      ...
      throw new IdentitiesExhaustedError(`Egress tier exhausted: no proxy identities available`, 'egress');
    }
...
async function executeAuthenticated(
  ...
  for (let attempt = 0; attempt < TIER4_MAX_AUTH_ATTEMPTS; attempt++) {
    ...
    if (!authAcq) {
      ...
      throw new IdentitiesExhaustedError(`Authenticated tier exhausted: no credentials available for ${platform}`, platform);
    }
...
function classifyErrorForTierTransition(
  ...
  if (error instanceof GeoBlockedError || error instanceof DatacenterBlockedError || error instanceof CircuitBreakerOpenError) {
    if (capabilities.supportsEgressFallback) {
      return {
        nextTier: 'EGRESS',
        reason: error instanceof CircuitBreakerOpenError ? 'circuit_breaker_open' : (error instanceof GeoBlockedError ? 'geo_blocked' : 'datacenter_blocked'),
      };
    }
